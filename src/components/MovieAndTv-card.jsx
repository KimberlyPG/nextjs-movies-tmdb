...
const MovieAndTvCard = ({ data, name, type }) => {
    ...
    return (
        <div className="relative hover:opacity-70 h-full cursor-pointer overflow-hidden" onClick={() => navigate(`/details/${name}`, {state: {contentId: data.id, type: type }})}>
            ...
            {data.poster_path.original !== undefined ? (
                <img 
                    className="h-full rounded-xl hover:scale-110"
